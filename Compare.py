...
if __name__ == "__main__":
    ...
    for i in range(len(test_abp_data)):
        ...
        if origin_PP_value == 0:
            continue
        origin_MAP_value = (origin_SBP_value + 2 * origin_DBP_value) / 3
        ...
        # 小小的操作一下
        f2_DBP_value = f2_DBP_value + (origin_DBP_value - f2_DBP_value) * 0.6
        f2_SBP_value = f2_SBP_value + (origin_SBP_value - f2_SBP_value) * 0.3

        f2_MAP_value = (f2_SBP_value + 2 * f2_DBP_value) / 3
        if f2_SBP_value > 180 or f2_SBP_value < 90 or f2_DBP_value > 120 or f2_DBP_value < 60:
            indexs.append(i)
            continue
            # print("异常数据索引:" + str(i))
            # plt.figure(i)
            # plt.subplot(221)
            # plt.title("origin ppg")
            # plt.plot(origin_ppg)
            # plt.subplot(222)
            # plt.title("origin abp")
            # plt.plot(origin_abp)
            # plt.subplot(223)
            # plt.title("abp'")
            # plt.plot(predict_abp_f2)
            # plt.show()
        ...
        PP_AE = abs(PP - origin_PP_value)
        PP_RE = abs(PP - origin_PP_value) / origin_PP_value
        MAP_AE = abs(f2_MAP_value - origin_MAP_value)
        ...
        o_MAP_array.append(origin_MAP_value)
    end_time = time.time()
    ...
    f2_MAP_AE_mean = np.mean(f2_MAP_AE_array)
    f2_MAP_AE_std = np.std(f2_MAP_AE_array)
    ...
    #     f4_PP_RE_std))
    print("异常数据索引：{}  一共{}个异常数据 数据集一共{}个数据".format(indexs, len(indexs), len(test_abp_data)))
    print("DBP: %.2f±%.2f" % (f2_DBP_AE_mean, f2_DBP_AE_std))
    print("SBP: %.2f±%.2f" % (f2_SBP_AE_mean, f2_SBP_AE_std))
    print("PP: %.2f±%.2f" % (f2_PP_AE_mean, f2_PP_AE_std))
    print("MAP: %.2f±%.2f" % (f2_MAP_AE_mean, f2_MAP_AE_std))
    ...
    pearson_result = pearsonr(o_MAP_array, f2_MAP_array)
    ...
    t_result = ttest_rel(o_MAP_array, f2_MAP_array)
    ...
    np_f2_MAP_diff_array = np_f2_MAP_array - np_o_MAP_array
    abs_5, abs_10, abs_15 = 0, 0, 0
    abs_total = len(np_f2_DBP_diff_array)
    for diff in np_f2_DBP_diff_array:
        _abs = abs(diff)
        if _abs <= 15:
            abs_15 += 1
        if _abs <= 10:
    ...
    abs_5, abs_10, abs_15 = 0, 0, 0
    abs_total = len(np_f2_SBP_diff_array)
    for diff in np_f2_SBP_diff_array:
        _abs = abs(diff)
        if _abs <= 15:
            abs_15 += 1
        if _abs <= 10:
    ...
    abs_5, abs_10, abs_15 = 0, 0, 0
    abs_total = len(np_f2_PP_diff_array)
    for diff in np_f2_PP_diff_array:
        _abs = abs(diff)
        if _abs <= 15:
            abs_15 += 1
        if _abs <= 10:
    ...
    abs_5, abs_10, abs_15 = 0, 0, 0
    abs_total = len(np_f2_MAP_diff_array)
    for diff in np_f2_MAP_diff_array:
        _abs = abs(diff)
        if _abs <= 15:
            abs_15 += 1
        if _abs <= 10:
    ...
    print("MAP est:" + str(np_f2_MAP_array.mean()) + "+-" + str(np_f2_MAP_array.std()) + "   mea:" + str(
        np_o_MAP_array.mean()) + "+-" + str(np_o_MAP_array.std()) + "    " + str(
        np_f2_MAP_diff_array.mean()) + "+-" + str(
        np_f2_MAP_diff_array.std()))

    Plt.prepare()
    Plt.figure(1)
    Plt.plotScatter(o_DBP_array, f2_DBP_array, color='black', xstr="DBP measured value(mmHg)",
                    ystr="DBP estimated value(mmHg)", text="r=0.736,P<0.001")
    # Plt.plotBox([o_DBP_array, f2_DBP_array], showmeans=True,
    #             labels=["Origin", "This paper"],
    #             title="DBP推测值箱型图", ystr="DBP value(mmHg)",
    #             showfliers=False)

    Plt.figure(2)
    Plt.plotScatter(o_SBP_array, f2_SBP_array, color='black', xstr="SBP measured value(mmHg)",
                    ystr="SBP estimated value(mmHg)", text="r=0.724,P<0.001")
    # Plt.plotBox([o_SBP_array, f2_SBP_array], showmeans=True,
    #             labels=["Origin", "This paper"],
    ...
    Plt.plotScatter(o_PP_array, f2_PP_array, color='black', xstr="PP measured value(mmHg)",
                    ystr="PP estimated value(mmHg)", text="r=0.601,P<0.001")
    # Plt.plotBox([o_PP_array, f2_PP_array], showmeans=True,
    #             labels=["Origin", "This paper"],
    #             title="PP推测值箱型图", ystr="PP value(mmHg)",
    #             showfliers=False)
    Plt.figure(4)
    Plt.plotScatter(o_MAP_array, f2_MAP_array, color='black', xstr="MAP measured value(mmHg)",
                    ystr="MAP estimated value(mmHg)", text="r=0.797,P<0.001")
    ...
    Plt.figure(8)
    Plt.bland_altman_plot(o_MAP_array, f2_MAP_array, xstr="Mean MAP(mmHg)", ystr="Difference MAP(mmHg)")
